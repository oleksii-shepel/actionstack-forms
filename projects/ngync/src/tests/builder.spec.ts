...
describe('builder', () => {
  ...
  it('should build form with options', () => {
    let model = { a: 1, b: 2, c: 3 };
    let form = buildForm({model, options: {__group: {}, a: {validators: Validators.required}, b: {validators: Validators.email}, c: {validators: Validators.maxLength(10)} }});

    expect(form.value).toEqual(model);
  });
  it('should build form array with options', () => {
    let model = [1, 2, 3];
    let form = buildForm({model, options: { __group: {}, '0': {validators: Validators.required}, '1': {validators: Validators.email}, '2': {validators: Validators.maxLength(10)} }});
